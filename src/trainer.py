...
class train_callback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        args = self.args
        to_save_dict = {}
        epoch_loss = trainer.my_epoch_loss
        ...
        if trainer.is_global_zero:  # logging
            ...
            trainer.my_loss_sum = 0
            trainer.my_loss_count = 0
            if (trainer.current_epoch + 1) >= args.epoch_count:
                import matplotlib.pyplot as plt

                print(f"len(train_loss_list): {len(self.train_loss_list)}")
                print(f"len(val_loss_list): {len(self.val_loss_list)}")

                # ---------- 畫訓練 loss 圖 ----------
                train_losses = [x.item() if torch.is_tensor(x) else x for x in self.train_loss_list]
                val_losses = [x.item() if torch.is_tensor(x) else x for x in self.val_loss_list]
                ...
                plt.legend()
                plt.grid()
                plt.savefig(f"{args.proj_dir}/train_loss_curve.png")
                ...
                plt.legend()
                plt.grid()
                plt.savefig(f"{args.proj_dir}/val_loss_curve.png")
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if val_loss is not None:
            ...
            if trainer.is_global_zero:
                trainer.my_log.write(f"[VAL] Epoch {trainer.current_epoch} val_loss: {val_loss:.6f}\n")
                trainer.my_log.flush()
