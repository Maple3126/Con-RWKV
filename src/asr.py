...
class SLAM_ASR(pl.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.encoder_time_sum = 0.0
        self.encoder_time_count = 0
        self.args = args
        self._device = device
        self.val_loss_sum = 0
    ...
    def _prepare_input_embeds(
        ...
        """
        import time 
        torch.cuda.synchronize()  # 確保之前 CUDA 都完成
        start_time = time.time()

        speech_output, mask = self.speech_encoder(audios)

        torch.cuda.synchronize()  # 等待 encoder 結束
        end_time = time.time()
        encoder_time = end_time - start_time
        self.encoder_time_sum += encoder_time
        self.encoder_time_count += 1
    ...
    def forward(self, audios: List[float], transcriptions: List[str] = None):
        ...
        outputs = self.language_model(inputs_embeds=prompt_embed) #outputs, dummy_losses
        
        return outputs[0], true_labels, prompt_mask #dummy_losses
    ...
    def training_step(self, batch, batch_idx):
        #try
            ...
            #     loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))
                ...
                logits, targets, mask = self(idx, transcription)  #dummy_losses
                mask = mask.view(-1)
                sum_mask = torch.sum(mask).item()
                ######
                ...
                # loss = loss + args.conv_#dummy_scale * #dummy_loss
                # print(#dummy_loss)      
            return L2Wrap.apply(loss, logits)
        
        # except RuntimeError as e:
        #      if "out of memory" in str(e):
        #         print(f"[OOM] Skipping batch {batch_idx}")
        #         torch.cuda.empty_cache()
        #     return L2Wrap.apply(loss, logits)
        #         dummy_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
        #         dummy_logits = torch.zeros((1, 1), device=self.device, requires_grad=True)
        #         return L2Wrap.apply(dummy_loss, dummy_logits)
    ...
    def configure_optimizers(self):
        ...
        # for n in param_dict:
        #     if "depthwise_conv" in n:
        #         print("[Check] depthwise_conv in param_dict:", n)
    ...
    #         grad = block.depthwise_conv.weight.grad
    #         # print(f"[Grad Check] Block {i} grad = {grad}")
    @property
    def config(self):
    ...
    def validation_step(self, batch, batch_idx):
        idx = [item[0] for item in batch]
        transcription = [item[1] for item in batch]
        # idx, transcription, mask = batch   ###################
        # print("debug:", self(idx, transcription))
        logits, targets, mask, _ = self(idx, transcription)
    ...
    def on_validation_epoch_start(self):
        ...
        self.encoder_time_sum = 0
        self.encoder_time_count = 0
    
    def on_validation_epoch_end(self):
        avg_val_loss = self.val_loss_sum / self.val_loss_count if self.val_loss_count > 0 else 0
        self.log("val_loss", avg_val_loss, prog_bar=True, on_step=False, on_epoch=True)
        if self.encoder_time_count > 0:
            avg_encoder_time = self.encoder_time_sum / self.encoder_time_count
            print(f"[平均 Encoder 時間] {avg_encoder_time:.4f} 秒")
