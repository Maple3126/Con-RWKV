...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if args.conv:
            self.depthwise_conv = nn.Conv1d(
                args.n_embd, args.n_embd, kernel_size=args.conv_kernel, padding=0, groups=args.n_embd, dilation=1 # depthwise
            )
            self.kernel_size = args.conv_kernel
        else:
    ...
    if os.environ["RWKV_TRAIN_TYPE"] == 'infctx':
        ...
        def forward(self, x, x_emb=None):
            ...
            # dummy_loss = torch.tensor(0.0, device=x.device, dtype=x.dtype)
            # if self.training:
            #      dummy_loss = conv_out.square().mean()  ---8/19
