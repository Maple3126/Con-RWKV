...
class RWKV_Tmix_x060_state(MyModule):
    ...
    def forward(self, x):
        ...
        # x = x.permute(0, 2, 1)
        # conv=nn.Conv1d(2048,2048,64)
        # torch.save(conv.state_dict(), 'output/conv_weights.pth')
        # x = conv(x)
        # x = x.permute(0, 2, 1)
        # print(x.shape)   ###看形狀
        return self.jit_func_2(x, g)
...
class RWKV_Tmix_x060_infctx(MyModule):
    def __init__(self, args, layer_id):
        ...
        self.time_shift = nn.ZeroPad2d((0, 0, 1, -1))  #不補 #不補 #第一列0 #刪最後一列
        self.receptance = make_linear_att(args.n_embd, args.dim_att, bias=False)
        self.key = make_linear_att(args.n_embd, args.dim_att, bias=False)
    ...
    def jit_func(self, x, shift_state):
        B, T, C = x.size()
        xx = torch.concat((shift_state.unsqueeze(1), x[:, :-1]), dim=1) - x  # unsqueeze() 把1插在第幾位置
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        self.ln_even = nn.LayerNorm(args.n_embd)
        # self.conv_head = nn.Sequential(
        #     nn.Linear(args.n_embd, args.n_embd),
        #     nn.ReLU(),
        #     nn.Linear(args.n_embd, args.vocab_size) )
        # # Pointwise conv to double channels for GLU
        # self.pointwise_conv1 = nn.Conv1d(args.n_embd, args.n_embd * 2, kernel_size=1)

        # Depthwise conv (causal optional)
        if args.conv:
            self.depthwise_conv = LocalAttention(
                window_size=args.conv_kernel,
                attn_dim=args.n_embd,
                causal=True,
                autopad=True, 
                use_rotary_pos_emb=True  # 如果你沒開位置編碼就設 False
            )
        else:
            self.depthwise_conv = None
        self.batch_norm = nn.BatchNorm1d(args.n_embd)
        self.activation = nn.SiLU()  # or nn.Swish()
        
        # # Pointwise conv to restore dimension
        # self.pointwise_conv2 = nn.Conv1d(args.n_embd, args.n_embd, kernel_size=1)
        ...
        # self.conv = nn.Conv1d(
        #     in_channels=args.n_embd,
        #     out_channels=args.n_embd,
        #     kernel_size=15,
        #     bias=False,
        #     padding=7,
        #     groups=args.n_embd  
        ...
        def forward(self, x, x_emb=None):
            ...
            if self.args.dropout == 0:
                ...
                if self.depthwise_conv is not None:
                    residual = x
                    conv_out = self.depthwise_conv(x, x, x)  # q=k=v=x
                    x = self.layer_norm(conv_out)
                    x = residual + x
                #     conv_logits = self.conv_head(conv_out)  # shape: [B, T, V]
                # else:
                #     conv_logits = None
            ...
            # def local_contrastive_loss(conv_out):
            #     """
            #     conv_out: Tensor of shape [B, T, C]
            #     Returns:
            #         scalar contrastive loss (L_pos + L_neg)
            #     """
            #     # B, T, C = conv_out.shape

            #     # Positive pair: (C_t, C_{t+1})
            #     pos = conv_out[:, :-1, :]         # [B, T-1, C]
            #     anchor_pos = conv_out[:, 1:, :]   # [B, T-1, C]
            #     L_pos = ((anchor_pos - pos)**2).mean()

            #     # Negative pair: (C_t, C_{t+2})
            #     neg = conv_out[:, :-2, :]         # [B, T-2, C]
            #     anchor_neg = conv_out[:, 2:, :]   # [B, T-2, C]
            #     L_neg = -((anchor_neg - neg)**2).mean()  # 越遠越好 → maximize distance → minimize -distance

            #     # Combine
            #     L_contrastive = L_pos + L_neg
            #     return L_contrastive
            
            # def local_contrastive_loss(conv_out, temperature=0.07):
            #     """
            #     conv_out: Tensor of shape [B, T, C]
            #     Returns:
            #         scalar contrastive loss using cosine similarity + softmax (InfoNCE style)
            #     """

            #     B, T, C = conv_out.shape

            #     # Positive pairs: (C_t, C_{t+1})
            #     anchor = conv_out[:, :-1, :]       # [B, T-1, C]
            #     positive = conv_out[:, 1:, :]      # [B, T-1, C]

            #     # Normalize embeddings (cosine similarity)
            #     anchor = F.normalize(anchor, dim=-1)
            #     positive = F.normalize(positive, dim=-1)

            #     # Compute similarity matrix: anchor vs all positive (batchwise)
            #     logits = torch.matmul(anchor, positive.transpose(1, 2)) / temperature  # [B, T-1, T-1]
            #     labels = torch.arange(T-1, device=conv_out.device).repeat(B, 1)  # correct pos: diagonal
            #     loss = F.cross_entropy(logits, labels, reduction='mean')

            #     return loss
            
            dummy_loss = torch.tensor(0.0, device=x.device, dtype=x.dtype)
            if self.training:
                 dummy_loss = conv_out.square().mean()
            # L_contrastive = local_contrastive_loss(conv_out)
     
            # print("[Check] dummy_loss requires_grad =", dummy_loss.requires_grad)
            # print("[Check] depthwise_conv weight requires_grad =", self.depthwise_conv.weight.requires_grad)
            return x , dummy_loss  # L_contrastive 
...
class RWKV(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if "depthwise_conv" in n and p.requires_grad
        ]
        ...
        # conv_params = [p for n, p in self.named_parameters() if 'depthwise_conv' in n and p.requires_grad]
        # if len(conv_params) > 0:
        #     optim_groups.append({"params": conv_params, "weight_decay": 0.0, "my_lr_scale": 1.0})    
        param_dict = {n: p for n, p in self.named_parameters()}
    ...
    if os.environ["RWKV_TRAIN_TYPE"] == 'infctx':
        ...
        def training_step(self, batch, batch_idx):
            ...
            def checkpointed_step(idx, targets, prev_loss, last_shift_states,
                ...
                # loss = loss + 0.01 * sum(b._dummy_loss for b in self.blocks if hasattr(b, "_dummy_loss")) XXXX
                ########################
        ...
        def forward(self, idx=None, inputs_embeds = None):
                ...
                for block in self.blocks:
                    if args.grad_cp == 1:
                        if args.lora or args.state_tune or args.train_type == 'state':
                            x, d_loss = torch_checkpoint(block, x, x_emb, use_reentrant=False)
                        else:
                            x, d_loss = deepspeed.checkpointing.checkpoint(block, x, x_emb)
                    else:
                        x, d_loss = block(x, x_emb)
                    dummy_losses.append(d_loss)
                    # contra_losses.append(d_loss)
            else:
                for block in self.blocks:
                    if args.grad_cp == 1:
                        if args.lora or args.state_tune or args.train_type == 'state':
                            ...
                            x, d_loss = deepspeed.checkpointing.checkpoint(block, x)
                    else:
                        x, d_loss = block(x)
                    dummy_losses.append(d_loss)
                    # contra_losses.append(d_loss)
            ...
            if args.head_qk > 0:
                ...
                x = self.head(x)
            # print("gg")
            return x, dummy_losses #contra_losses 

        def generate(self, tokenizer, idx=None, inputs_embeds=None,):
            MAX_LENGTH = 100
            output_seq, _ = self(idx,inputs_embeds=inputs_embeds)#调用模型
            ...
            for i in range(MAX_LENGTH):
                ...
                output_seq, _ = self(idx, inputs_embeds=inputs_embeds)
        ...
        def training_step(self, batch, batch_idx):
            ...
            if args.loss_mask:
                ...
                logits, dummy_losses = self(idx) ##################################contra_losses
                ...
                dummy_loss = sum(dummy_losses)
                # contra_loss = torch.stack(contra_losses).mean()
                # loss = loss + args.conv_dummy_scale * dummy_loss #contra_loss
                # self.log("contra_loss", contra_loss, prog_bar=True, on_step=True)
                # print("b") 
            return L2Wrap.apply(loss, logits)       
            # return {"loss": L2Wrap.apply(loss, logits), "contra_loss": contra_loss}
