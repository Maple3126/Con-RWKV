...
if __name__ == "__main__":
    ...
    parser.add_argument("--epoch_steps", default=40, type=int)  # a mini "epoch" has [epoch_steps] steps
    ...
    #conv
    parser.add_argument("--conv", action="store_true", help="啟用 depthwise conv block")
    parser.add_argument("--conv_kernel", type=int, default=5, help="Conv1d kernel size")
    parser.add_argument("--conv_dummy_scale", type=float, default=0.0, help="dummy loss 係數")
    ...
    args.check_val_every_n_epoch = int(1)
...
"""
    ...
    if args.state_tune or args.train_type=='state':
        for name, module in model.named_modules():
            for pname, param in module.named_parameters():
                if 'state' in pname  :
                    param.requires_grad = True  #挑一些解凍
            break
        for name, module in model.named_modules():
            for pname, param in module.named_parameters():
                if 'depthwise_conv' in pname:
                    param.requires_grad = True
                    # print(f"[CHECK] {pname} requires_grad = {param.requires_grad}")####
    ...
    # conv_paths = glob.glob('output/conv_weights*.pth') #### my code
    # if conv_paths:
    #     conv_path = conv_paths[0]
    #     Total_model.load_state_dict(torch.load(conv_path), strict=False)
    # else:
    ...
    if(args.op == "train"):# training
        ...
        ''''''
        dataset = MyDataset(args, split="train") #training
        data_loader = DataLoader(dataset, shuffle=True, pin_memory=True, batch_size=args.micro_bsz, num_workers=4, persistent_workers=True, drop_last=True, collate_fn=lambda x: x)
        val_dataset = MyDataset(args, split="valid")  # 或是指定 val 資料夾路徑
        val_loader = DataLoader(val_dataset, shuffle=False, batch_size=args.micro_bsz, num_workers=4, drop_last=False, collate_fn=lambda x: x)
        ''''''
        # from src.train_cache import load_cached_dataloader
        # data_loader = load_cached_dataloader(args.cache_dir, args.micro_bsz)
        # # val_loader = load_cached_dataloader(args.val_cache_dir, args.micro_bsz)
        print("train starting...")
        trainer.fit(Total_model, data_loader, val_loader)  #trainer.fit(Total_model, data_loader, val_loader)
    ...
    #         print("\n\n")
        ...
        test_dataset_c = MyDataset(args, split="test_c")  # 或是指定 val 資料夾路徑
        test_dataset_oth = MyDataset(args, split="test_oth")
        # test_loader_c = DataLoader(test_dataset_c, shuffle=False, batch_size=args.micro_bsz, num_workers=4, drop_last=False, collate_fn=lambda x: x)
        # test_loader_oth = DataLoader(test_dataset_oth, shuffle=False, batch_size=args.micro_bsz, num_workers=4, drop_last=False, collate_fn=lambda x: x)
        # ds1 = load_dataset("librispeech_asr","clean",split="test")
        # ds2 = load_dataset("librispeech_asr","other",split="test")
        dss = [test_dataset_c,test_dataset_oth]
