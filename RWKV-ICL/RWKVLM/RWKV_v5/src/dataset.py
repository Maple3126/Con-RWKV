...
class ValDataset(MyDataset):
    def __init__(self, args):
        # 建立一份 args 的副本，不影響原本訓練用的設定
        args_copy = argparse.Namespace(**vars(args))
        self.args = args
        # 你原本有的初始化邏輯

        # ✅ 加這三行（模仿 train dataset）
        self.global_rank = 0
        self.real_epoch = 0
        self.world_size = 1

        # 將副本中的 data_file 改成 data_val_file（若沒設定就報錯）
        args_copy.data_file = getattr(args, "data_val_file", None)
        if args_copy.data_file is None:
            raise ValueError("ValDataset 需要指定 --data_val_file")

        # 呼叫原本的 MyDataset 初始化流程（使用改寫過的 args_copy）
        super().__init__(args_copy)
