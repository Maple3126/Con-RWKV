...
class train_callback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if trainer.is_global_zero:  # logging
            ...
            # 檢查是否有 conv_kernel
            conv_str = f"{args.conv_kernel}" if hasattr(args, "conv_kernel") else "N/A"

            trainer.my_log.write(
                f"{args.epoch_begin + trainer.current_epoch} {trainer.my_epoch_loss:.6f} "
                f"{math.exp(trainer.my_epoch_loss):.4f} VAL={val_loss_str} {conv_str} "
                f"{trainer.my_lr:.8f} {datetime.datetime.now()} {trainer.current_epoch}\n"
            )
            trainer.my_log.flush()
