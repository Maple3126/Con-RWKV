...
# wkv6_cuda = load(
#     name="wkv6",
#     sources=["cuda/wkv6_op.cpp", "cuda/wkv6_cuda.cu"],
#     verbose=True,
#     extra_cuda_cflags=[
...
#         f"-D_T_={args.ctx_len}"
#     ],
#     is_python_module=False  # ← ⚠️ 這個參數千萬不能被錯寫在 cflags 裡面
# )
import traceback
try:
    wkv6_cuda = load(
...
# 檢查型別
if isinstance(wkv6_cuda, str):
    print("❌ load() 仍回傳 str，錯誤內容：")
    print(wkv6_cuda)
    exit()

#wkv6_cuda = load(name="wkv6", sources=["cuda/wkv6_op.cpp", f"cuda/wkv6_cuda.cu"],
                #verbose=True, extra_cuda_cflags=["-res-usage", "--use_fast_math", "-O3", "-Xptxas -O3", "--extra-device-vectorization", f"-D_N_={args.head_size_a}", f"-D_T_={args.ctx_len}", is_python_module=False])
...
model_params = torch.load("/home/RWKVICL/RWKVLM/RWKV_v5/out/L2-D768-x060/rwkv-final.pth", map_location="cpu")
tokenizer = TinyBinaryTokenizer()
...
# 產生 Markov 鏈資料
# def generate_markov_data(order=2, length=50, vocab=[0, 1], beta=1.0, seed=None):
#     if seed: np.random.seed(seed)
#     contexts = [tuple(map(int, format(i, f'0{order}b'))) for i in range(len(vocab) ** order)]
#     transitions = {ctx: np.random.dirichlet([beta] * len(vocab)) for ctx in contexts}
#     seq = list(np.random.choice(vocab, size=order))
#     for _ in range(length - order):
#         ctx = tuple(seq[-order:])
#         next_token = np.random.choice(vocab, p=transitions[ctx])
#         seq.append(next_token)
#     return np.array(seq, dtype=np.uint16), transitions
def generate_random_transition(order=2, vocab=[0, 1], beta=1.0, seed=42):
    if seed is not None:
        np.random.seed(seed)
    ...
    for ctx in contexts:
        transitions[ctx] = np.random.dirichlet([beta] * len(vocab))  # e.g. [1, 1] for uniform

    return transitions



def generate_markov_data(order=2, length=50, vocab=[0, 1], beta=1.0, seed=42):
    transitions = generate_random_transition(order, vocab, beta, seed)
    # 初始化序列
    if seed is not None:
        np.random.seed(seed + 999)  # 避免和 transition 用同一亂數
    seq = list(np.random.choice(vocab, size=order))
...
order = 3
seq_len = 2000000
test_seq, true_transitions = generate_markov_data(order=order, length=seq_len, seed=20)  #42 good 20 bad  # 

# input_tokens = torch.tensor(test_seq[:seq_len], dtype=torch.long).unsqueeze(0).cuda()  # shape [1,T]
...
#     input_tokens = test_seq[:order + seq_len]  # 前22 token，讓我們可以看到從第3個token開始的20個預測
#     input_tensor = torch.tensor(input_tokens, dtype=torch.long).unsqueeze(0).cuda()

#     logits = model(input_tensor)[0]  # shape: [T, vocab_size]
#     probs = F.softmax(logits, dim=-1).float().detach().cpu().numpy()

#     print("\n=== Context-wise model prediction per token ===")
#     for i in range(order, len(input_tokens)):
#         ctx = tuple(input_tokens[i-order:i])
#         model_prob = probs[i]  # 第 i 個 token 的預測出現在 logits[i-1]
#         true_prob = true_P[ctx]
#         ctx_str = tuple(int(x) for x in ctx)
#         print(f"Step {i:2d} | Context: {ctx_str} =>  Model P(0)={model_prob[0]:.2f}, P(1)={model_prob[1]:.2f}   |  True P(0)={true_prob[0]:.2f}, P(1)={true_prob[1]:.2f}")
#     # KL 計算
#     kl_list = []
#     gt_tokens = []
#     pred_tokens = []

#     for i in range(order, seq_len):
#         ctx = tuple(test_seq[i - order:i])
#         true_probs = true_P[ctx]
#         pred_probs = probs[i]  # logits[i-1]是對於第i個token的預測
#         kl = sum(rel_entr(true_probs, pred_probs))
#         kl_list.append(kl)

#         gt_tokens.append(test_seq[i])
#         pred_tokens.append(np.argmax(pred_probs))

#     # 顯示結果
#     print("\n=== Context-wise Transition Probabilities ===")

#     print("Ground Truth:", ''.join(map(str, gt_tokens)))
#     print("Model Output:", ''.join(map(str, pred_tokens)))
#     print(f"Average KL Divergence over {len(kl_list)} steps: {np.mean(kl_list):.4f}")
########################
...
with torch.no_grad():
    ...
    context_counts = defaultdict(lambda: [0, 0])  # [n0, n1]
    beta = 1.0

    print("\n=== Online Laplace vs Model Probabilities ===")
    kl_list = []
    gt_tokens = []
    pred_tokens = []

    for i in range(order, len(test_seq)):
        ctx = tuple(int(x) for x in test_seq[i - order:i])
        nxt = test_seq[i]

        # 即時計算 Laplace smoothing
        n0, n1 = context_counts[ctx]
        total = n0 + n1
        p1 = (n1 + beta) / (total + 2 * beta)
        p0 = 1 - p1
        laplace_prob = [p0, p1]
        ...
        # 印出預測比較
        if i >= len(test_seq) - 50 :
            print(f"Step {i:2d} | Context: {ctx} =>  Model P(0)={model_prob[0]:.2f}, P(1)={model_prob[1]:.2f}   |  Laplace P(0)={p0:.4f}, P(1)={p1:.2f}")

        # 記錄 KL divergence
        kl = sum(rel_entr(laplace_prob, model_prob))
        kl_list.append(kl)

        # 記錄 ground truth 與模型輸出
        gt = nxt
        pred = np.argmax(model_prob)
        gt_tokens.append(str(gt))
        pred_tokens.append(str(pred))

        # 更新 context 統計
        if nxt == 0:
            context_counts[ctx][0] += 1
        else:
            context_counts[ctx][1] += 1

# 印出整體結果
print("\n=== Context-wise Transition Probabilities ===")
print("Ground Truth:", ''.join(gt_tokens[:50]))
print("Model Output:", ''.join(pred_tokens[:50]))   #-50:
print(f"Average KL Divergence over {len(kl_list)} steps: {np.mean(kl_list):.4f}")
print("=== Transitions ===")
for ctx, probs in true_transitions.items():
    print(f"{ctx} => P(0)={probs[0]:.2f}, P(1)={probs[1]:.2f}")
